...
import { requestCalendarLink, requestChannelSelection, requestNotificationTime } from './requestInformation.js';
...
async function initiateDMSetup(guild, client, db) {
  ...
  try {
    ...
    const notificationTime = await requestNotificationTime(inviter, guild, db);
